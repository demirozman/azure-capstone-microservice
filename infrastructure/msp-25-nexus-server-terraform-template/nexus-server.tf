...
//Nexus server will run on Amazon Linux 2023 with custom security group
...
resource "null_resource" "forpasswd" {
  ...
  provisioner "local-exec" {
    command = "ssh -i ~/.ssh/${var.mykey}.pem -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ec2-user@${aws_instance.tf-nexus-server.public_ip} 'sudo docker cp nexus:/nexus-data/admin.password && cat /home/ubuntu/admin.password' > initialpasswd.txt"
  }
}
